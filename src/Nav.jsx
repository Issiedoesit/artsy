...
export default function NavBar(){
    ...
    const navToggle = () =>{
        ...
        if (NavRef.current.classList.contains('-translate-x-[150%]')){
            NavRef.current.classList.remove('-translate-x-[150%]')
        }else{
            NavRef.current.classList.add('-translate-x-[150%]')
        }
    }
    ...
    return(
        <nav className=' flex flex-row items-center justify-between w-full h-16 py-14 px-tenPercent'>
           <div className='block lg:hidden'>
            ...
            <ul ref={NavRef} className='fixed transition-transform duration-1000 -translate-x-[150%] lg:-translate-x-0 text-grey1x lg:text-grey2x top-0 bg-white lg:bg-transparent z-100 pt-40 pb-10 min-h-screen lg:h-auto px-20 lg:p-0 left-0 w-full lg:static flex flex-col lg:flex-row lg:items-center lg:justify-center gap-12'>
                <div className='flex items-center justify-between w-full top-10 left-0 px-20 py-5 fixed lg:hidden z-120'>
                    <img src={logo} alt="logo" className='max-w-28' />
                    <button onClick={navToggle}>
                ...
                <NavLink exact to='/' end={true} className={({isActive}) =>(isActive ? 'text-2xl font-satoshi-medium underline underline-offset-4' : 'text-2xl font-satoshi-regular hover:underline transition ease-in-out duration-[10s]')}>Home</NavLink>
                <NavLink exact to='/marketplace' className={({isActive}) =>(isActive ? 'text-2xl font-satoshi-medium underline underline-offset-4' : 'text-2xl font-satoshi-regular hover:underline transition ease-in-out duration-[10s]')}>Marketplace</NavLink>
                <NavLink exact to='/auctions' className={({isActive}) =>(isActive ? 'text-2xl font-satoshi-medium underline underline-offset-4' : 'text-2xl font-satoshi-regular hover:underline transition ease-in-out duration-[10s]')}>Auctions</NavLink>
                <NavLink exact to='/drop' className={({isActive}) =>(isActive ? 'text-2xl font-satoshi-medium underline underline-offset-4' : 'text-2xl font-satoshi-regular hover:underline transition ease-in-out duration-[10s]')}>Drop</NavLink>
                <button className='fixed right-16 bottom-12 z-150 block lg:hidden'>
            ...
            <ul className='flex flex-row items-center gap-8'>
                <button>
                    ...
                    <svg className='hover:animate-pulse' width="28" height="28" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <button>
                    ...
                    <svg className='hover:animate-pulse' width="28" height="28" viewBox="0 0 49 49" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <button>
                    <p className='hidden'>Notification</p>
                    <svg className='hover:animate-pulse' width="26" height="29" viewBox="0 0 46 49" fill="none" xmlns="http://www.w3.org/2000/svg">
